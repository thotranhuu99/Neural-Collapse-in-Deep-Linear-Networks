...
NUM_CLASS_SAMPLES = [200, 100, 50, 50]
NUM_SAMPLES = sum(NUM_CLASS_SAMPLES)


class SyntheticModel(nn.Module):
    def __init__(self, hidden, num_classes, depth_linear, NUM_SAMPLES, weight_decay, feature_decay):
        super().__init__()
        self.weight_decay = weight_decay
        self.feature_decay = feature_decay
        self.depth_linear = depth_linear
        self.H = nn.Parameter(torch.normal(0, 0.1, size=(
            NUM_SAMPLES, hidden)), requires_grad=True)
        fc = []
        for i in range(depth_linear-1):
            fc += [nn.Linear(hidden, hidden, bias=False)]

        fc += [nn.Linear(hidden, num_classes, bias=False)]
    ...
    def forward(self, Y):
        ...
        loss_mse = torch.norm(out - Y, p='fro') ** 2 / (2*NUM_SAMPLES)
        criterion = nn.CrossEntropyLoss()
        Y_label = torch.argmax(Y, dim=1)
        loss_CE = criterion(out, Y_label)
        loss_w = self.weight_decay / 2 * \
            sum([torch.sum(self.fc[i].weight**2)
                for i in range(self.depth_linear)])
        loss_h = self.feature_decay / 2 * torch.sum(self.H ** 2)
        loss = loss_mse + loss_w + loss_h
...
def compute_info(model, Y, targets):
    ...
    outputs, loss, features_list = model(Y)
    for i in range(len(model.fc)):
        for y in range(len(NUM_CLASS_SAMPLES)):
            ...
            if indexes.nelement() == 0:
                ...
                mu_c_dict_list[i][y] = mu_c_dict_list[i][y] / \
                    NUM_CLASS_SAMPLES[y]
        mu_G_list[i] = mu_G_list[i] / sum(NUM_CLASS_SAMPLES)
    prec1 = compute_accuracy(outputs.data, targets.data, topk=(1,))
    return mu_G_list, mu_c_dict_list, prec1[0].cpu().numpy(), features_list, loss
...
def compute_Sigma_W(H, mu_c_dict, targets):
    ...
    for y in range(len(NUM_CLASS_SAMPLES)):
        ...
        if indexes.nelement() == 0:
            ...
            Sigma_W += ((H[indexes, :] - mu_c_dict[y]).unsqueeze(2)
                        @ (H[indexes, :] - mu_c_dict[y]).unsqueeze(1)).sum(0)
    Sigma_W /= NUM_SAMPLES
...
def compute_NC2_W(W, s_array, depth):
    ...
    ETF = 1 / pow(K - 1, 0.5) * (torch.eye(K) - 1 / K * torch.ones((K, K))).cuda()
...
def compute_NC3_W_sub_H(W, mu_c_dict, s_array, feature_decay_rate, depth):
    ...
    mu_G = torch.zeros(1, mu_c_dict[0].shape[0], device=W.device)
    for i in range(K):
        H[:, i] = mu_c_dict[i]
...
def main(args):
    ...
    name = "imbalance-MSE-nobias" \
           + "-depth_linear_" + str(args.depth_linear) \
           + "-" + "lr_" + str(args.lr) + "-" + "dim_" + str(args.hidden) \
           + "-" + "epochs_" + str(args.num_iteration) + "-seed_" + str(args.seed)

    S_ARRAY = compute_s(args.weight_decay, args.feature_decay,
                        args.depth_linear, device="cuda")
    model = SyntheticModel(hidden=args.hidden, num_classes=args.num_classes,
        ...
        with torch.no_grad():
            ...
            for j in list(reversed(range(0, len(model.fc)-1))):
                if isinstance(model.fc[j], nn.Linear):
                    ...
                    W_list.append(W_temp)
            mu_G_train_dict, mu_c_dict_train_dict, train_acc1, features_list, loss = compute_info(
                model, Y, targets)
            Sigma_W = compute_Sigma_W(
                features_list[-1], mu_c_dict_train_dict[-1], targets)
            Sigma_B = compute_Sigma_B(
                mu_c_dict_train_dict[-1], mu_G_train_dict[-1])
            NC1_collapse_metric = np.trace(
                Sigma_W @ scilin.pinv(Sigma_B)) / len(mu_c_dict_train_dict[-1])
            print(NC1_collapse_metric)
            NC2_W = compute_NC2_W(W_list[-1], S_ARRAY, args.depth_linear)  # W_M...W_1
            NC2_H = compute_NC2_H(
                mu_c_dict_train_dict[-1], S_ARRAY, args.feature_decay, args.depth_linear)  # H only
            NC3_WH = compute_NC3_WH(
                W_list[0], mu_c_dict_train_dict[0], S_ARRAY, args.feature_decay, args.depth_linear)
            NC3_W_sub_H = compute_NC3_W_sub_H(
                W_list[-1], mu_c_dict_train_dict[-1], S_ARRAY, args.feature_decay, args.depth_linear)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--depth_linear', type=int, default=6)
